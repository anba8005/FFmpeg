...
static int write_header(struct AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i ++) {
    	...
    	if (stream->codecpar->codec_type != AVMEDIA_TYPE_AUDIO)
    		continue;
    	if (stream->codecpar->codec_id != AV_CODEC_ID_PCM_S16LE) {
...
static int write_packet(struct AVFormatContext *s, AVPacket *pkt)
{
	...
	c = s->priv_data;
	codec = s->streams[pkt->stream_index]->codecpar;
    ...
    // write pts
    pts = (float)pkt->pts / (float)90000;
    avio_printf(pb, "[ %.2f,",pts);
    // write stats
    AVRational tb = { 1, codec->sample_rate };
    int duration = av_rescale_q(pkt->duration,s->streams[pkt->stream_index]->time_base,tb);
    for (i = 0; i < codec->channels; i++) {
    	...
    	for (k = 0; k < duration; k++) {
    		...
    		sum += pow((double) sample, (double)2);
    	}
    	// find rms
    	rms = sqrt(sum / (double) duration);
    	//fprintf(stderr,"%f\n",rms);
    	result = map2range(float2db((float)rms / ((float) (1 << (sizeof(short) * 8 - 1)))), dbMin, dbMax, 0, 1) * 100;
    	// write rms
...
static const AVClass wfm_class = {
    .class_name = "WFM muxer",
    .item_name  = av_default_item_name,
    .option     = wfm_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

const AVOutputFormat ff_wfm_muxer = {
    .name              = "wfm",
